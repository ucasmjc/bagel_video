...
class Bagel(PreTrainedModel):
    ...
    def prepare_prompts(self, curr_kvlens, curr_rope, prompts, tokenizer, new_token_ids):
        ...
        generation_input = {
            "text_token_lens": torch.tensor(text_token_lens, dtype=torch.int,device=self.device),
            "packed_text_ids": torch.tensor(packed_text_ids, dtype=torch.long,device=self.device),
            "packed_text_position_ids": torch.tensor(packed_text_position_ids, dtype=torch.long,device=self.device),
    ...
    def forward_cache_update_text(
        ...
        #print("111111",packed_text_ids.device,self.language_model.model.embed_tokens.weight.device)
        packed_text_embedding = self.language_model.model.embed_tokens(packed_text_ids)
