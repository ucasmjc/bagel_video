...
from data.data_transform import CenterCropResizeVideo
class ImageTransform:
    def __init__(
        ...
        self.resize_video=CenterCropResizeVideo([min_image_size,min_image_size])
    ...
    def __call__(self, img, img_num=1):
        ...
        if img_num==1:
            img = self.resize_transform(img, img_num=1)
            img = self.to_tensor_transform(img)
        else:
            img = self.resize_video(img)
            img=img.float()/255.0
