...
from data.video_utils import FrameSampler
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            for item in dataset_names:
                ...
                if 'meta_path' in meta_info.keys():
                    # parquet/tar with json
                    with open(meta_info['meta_path'], 'r', encoding='utf-8') as f:
                        datasets_info=json.load(f)["datasets"]
                    if 'json_dir_list' not in dataset_args.keys():
                        dataset_args['json_dir_list'] = [ii[0] for ii in datasets_info]
                        dataset_args['image_dir_list'] = [ii[1] for ii in datasets_info]
                    else:
                        dataset_args['json_dir_list'].extend([ii[0] for ii in datasets_info])
                        dataset_args['image_dir_list'].extend([ii[1] for ii in datasets_info])

                if 'json_list_dir' in meta_info.keys():
                    # jsonl with jpeg
                    json_files=[os.path.join(meta_info['json_list_dir'],file_name) for file_name in os.listdir(meta_info['json_list_dir']) if ".json" in file_name and "key.json" not in file_name]
                    if 'json_dir_list' not in dataset_args.keys():
                        dataset_args['json_dir_list'] = json_files
                        dataset_args['image_dir_list'] = [meta_info['image_dir']]*len(json_files)
                    else:
                        dataset_args['json_dir_list'].extend(json_files)
                        dataset_args['image_dir_list'].extend([meta_info['image_dir']]*len(json_files))
                if 'tar_dir' in meta_info.keys():
                    # jsonl with jpeg
                    
                    tar_files = glob.glob(os.path.join(meta_info['tar_dir'],'*.tar')) 
    
                    if 'tar_dir_list' not in dataset_args.keys():
                        dataset_args['tar_dir_list'] = tar_files
                    else:
                        dataset_args['tar_dir_list'].extend(tar_files)
                if 'video_dir' in meta_info.keys():
                    if 'video_dir_list' not in dataset_args.keys():
                        dataset_args['video_dir_list'] = meta_info['video_dir']
                    else:
                        dataset_args['video_dir_list'].extend(meta_info['video_dir'])
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                # add a <|startofimage|> token
                sequence_status['packed_text_ids'].append(self.start_of_video)
                sequence_status['packed_text_indexes'].append(curr)
                curr += 1
                curr_split_len += 1
                C,T,H, W = image_tensor.shape
                ...
                # add a <|endofimage|> token
                sequence_status['packed_text_ids'].append(self.end_of_video)
                sequence_status['packed_text_indexes'].append(curr)
                # <|endofimage|> may have loss
                if item['special_token_loss'] == 1:
                    sequence_status['ce_loss_indexes'].append(curr)
...
if __name__=="__main__":
    ...
    dataset_config.max_latent_size = 32
