...
def create_sparse_mask(document_lens, split_lens, attn_modes, device):
    ...
    return and_masks(remove_noise_mask, sample_mask)

# mask1=create_sparse_mask([11],[3,4,4],['causal', 'noise',"full"],"cpu")
# mask_matrix = torch.zeros([11,11])
    
#     # 填充掩码矩阵
# for i in range(11):
#     for j in range(11):
#         # 使用固定的b=0, h=0（因为这些参数在掩码函数中未使用）
#         mask_matrix[i, j] = mask1(torch.tensor(0), 0, i, j)
# print(mask_matrix)
...
def get_flattened_position_ids_extrapolate(img_t,img_h, img_w, patch_size, max_num_patches_per_side):
    num_patches_h, num_patches_w = img_h // patch_size, img_w // patch_size
    coords_h = torch.arange(0, num_patches_h)
    coords_w = torch.arange(0, num_patches_w)
    pos_ids = (coords_h[:, None] * max_num_patches_per_side + coords_w).flatten().repeat(img_t)
    return pos_ids
...
def add_special_tokens(tokenizer):
    ...
    if '<|video_start|>' not in all_special_tokens:
        new_tokens.append('<|video_start|>')

    if '<|video_end|>' not in all_special_tokens:
        new_tokens.append('<|video_end|>')
    ...
    start_of_video = tokenizer.convert_tokens_to_ids('<|video_start|>')
    end_of_video = tokenizer.convert_tokens_to_ids('<|video_end|>')
