...
# def process_single_sub(item, folder):
#     """处理单个数据项的线程函数"""
...
#     except:
#         #print("error,",item["path"])
#         return None
#     return item
def process_single_sub(items, folder):
    """处理单个数据项的线程函数"""
    res=[]
    for item in items:
        tar_path=item["path"].split("/")[0]
...
def process_single_file(anno, folder, file_name, output_path,pre_fix, sub_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=sub_workers) as executor:
        ...
        for sub_id in range(0,length):
            ...
            futures.append(executor.submit(process_single_sub, sub_part, folder))
        last_part=data_out[length*part_length:]
        futures.append(executor.submit(process_single_sub, last_part, folder))
...
def filter_data(output_path):
    ...
    data=[
    ["/mnt/weka/data_hw/img_json/recap1", "/mnt/weka/data_hw/final/recap1"]
    ]
    newjson=[]
    ...
    for line in data:
        ...
        files = [anno]
        for f in files:
            filtered,origin=process_single_file(anno, folder, f, output_path, pre_fix,sub_workers=128)
            fnum=fnum+filtered
            onum=onum+origin
    print(fnum,onum)
